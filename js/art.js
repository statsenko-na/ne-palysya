...
(() => {
  ...
  function drawNorthWall(g) {
    ...
    note(g, 164, 91, 44, 12, ['ЧАЙ ЛИЧНЫЙ', '— БЛЕБ'], { tilt: 0.06, size: 5.5, bg: '#fbe98f' });
  ...
  function drawCounter(g) {
    ...
    // Банка «НА КОФЕ»
    R(g, 139, y - 12, 10, 12, 'rgba(210,230,235,0.6)'); R(g, 140, y - 4, 8, 4, '#c9a13a');
    note(g, 136, y - 22, 17, 8, ['НА', 'КОФЕ'], { noTape: true, size: 3.6 });
    // Чайник
  ...
  function drawDesk(g, d) {
    ...
    const st = d.style;
    if (st === 'crt') {
      crtBack(g, x + 54, y - 14); // ЭЛТ сдвинут вбок — Сиргея видно
      note(g, x + 4, y + 6, 18, 12, ['ПК НЕ', 'ВКЛ.'], { tilt: -0.1, size: 4.6, bg: '#fbe98f' });
      mug(g, x + 26, y + 14, '#a0382c');
    } else if (st === 'tigran') {
      // Тигран — дух офиса: флаги Армении и Казахстана, модель «Союза», стикер «БАЙКОНУР»
      const flag = (fx, fy, draw) => { R(g, fx - 1, fy, 1.2, 22, '#c8c2b0'); R(g, fx - 3, fy + 21, 6, 2, '#3a3a3a'); draw(fx, fy); };
      flag(x + 8, y - 14, (fx, fy) => { R(g, fx, fy, 16, 3.4, '#d90012'); R(g, fx, fy + 3.4, 16, 3.3, '#0033a0'); R(g, fx, fy + 6.7, 16, 3.3, '#f2a800'); });
      flag(x + 26, y - 14, (fx, fy) => {
        R(g, fx, fy, 16, 10, '#00afca');
        for (let k = 0; k < 5; k++) R(g, fx + 0.4, fy + 1 + k * 1.8, 1.4, 1, '#fec50c'); // орнамент у древка
        E(g, fx + 9, fy + 4, 2.2, 2.2, '#fec50c'); // солнце
        R(g, fx + 5.5, fy + 6.6, 7, 1, '#fec50c'); // орёл
      });
      // ракета на подставке
      const rx = x + 66, ry = y - 20;
      R(g, rx - 5, ry + 26, 10, 3, '#3a3a3a');
      R(g, rx - 2.5, ry + 4, 5, 22, '#eeeae0'); R(g, rx - 2.5, ry + 4, 1.2, 22, '#fff');
      g.fillStyle = '#c8322a'; g.beginPath(); g.moveTo(rx - 2.5, ry + 4); g.lineTo(rx, ry - 2); g.lineTo(rx + 2.5, ry + 4); g.fill();
      R(g, rx - 5, ry + 18, 2.5, 8, '#8a8f94'); R(g, rx + 2.5, ry + 18, 2.5, 8, '#8a8f94');
      R(g, rx - 2.5, ry + 12, 5, 1.2, '#c8322a');
      ...
      mug(g, x + 72, y + 6);
    } else if (st === 'majikboss') {
      monitorBack(g, x + 22, y - 14, 30, 20, '#343a3f');
      note(g, x + 56, y + 3, 24, 12, ['НЕ ОРАТЬ', 'ДО 10:00'], { size: 4.4, bg: '#f0c8c8', fg: '#b3261e' });
      papers(g, x + 6, y + 10, 4); // стопка «просрочки» Маджикистана
    } else {
      ...
      if (st === 'calc') { // Блеб — дата-сайентист: ноутбук с графиком обучения и стикер «ROC 0.51»
    ...
    if (d.owner) {
      const cw = WD.people.find(c => c.id === d.owner);
      const name = cw ? cw.name.toUpperCase() : 'БЫКЕНТИЙ';
  ...
  function buildProps() {
    ...
    prop(14, FT - 44, 44, 64, FT + 18, g => {
      ...
      note(g, x + 3, y + 25, 25, 11, ['НЕ ТРОГАТЬ!', '— ХЛАД'], { tilt: -0.06, size: 4.6, fg: '#b3261e' });
    ...
    // Синий биотуалет посреди банка — стёб над «ремонтом санузла»
    prop(240, 436, 56, 76, 508, g => drawCabin(g), 'wc_cabin');
  }

  function drawCabin(g) {
    const x = 248, y = 446, w = 38;
    R(g, x - 2, 506, w + 4, 4, 'rgba(0,0,0,0.3)'); // тень
    // крыша
    R(g, x - 1, y, w + 2, 9, '#dfe6ec'); R(g, x - 1, y, w + 2, 2, '#f4f8fb'); R(g, x - 1, y + 8, w + 2, 1.5, '#9aa8b4');
    R(g, x + 28, y - 7, 4, 8, '#9aa8b4'); R(g, x + 27, y - 8, 6, 2, '#6d7a86'); // вентиляция
    R(g, x + 6, y - 5, 8, 6, '#f5f2ea'); E(g, x + 10, y - 5, 4, 1.5, '#fffdf6'); E(g, x + 10, y - 5, 1.2, 0.6, '#b8b0a0'); // рулон на крыше
    // корпус с рёбрами
    R(g, x, y + 9, w, 53, '#2266c8');
    R(g, x, y + 9, 3, 53, '#184c9a'); R(g, x + w - 3, y + 9, 3, 53, '#184c9a');
    for (let i = 0; i < 5; i++) R(g, x + 4 + i * 7.5, y + 11, 1, 49, 'rgba(10,30,80,0.25)');
    // дверь
    R(g, x + 6, y + 12, 26, 49, '#2a74dc'); R(g, x + 6, y + 12, 26, 1.2, '#5a9af0'); R(g, x + 31, y + 12, 1, 49, '#154588');
    for (let i = 0; i < 4; i++) R(g, x + 10 + i * 5, y + 14, 3, 1, '#154588'); // вентиляционные щели
    E(g, x + 19, y + 24, 4, 4, '#f4f6fa'); E(g, x + 19, y + 22.6, 1.3, 1.3, '#2a74dc'); R(g, x + 17.6, y + 24.2, 2.8, 3, '#2a74dc'); // значок человечка
    R(g, x + 8, y + 30, 22, 7, '#f4f6fa'); T(g, 'БИОТУАЛЕТ', x + 19, y + 32.2, 3.3, '#154588', 'center', 700, FONT_SANS);
    T(g, 'ТОЛЬКО СОТРУДНИКИ', x + 19, y + 35.2, 2.4, '#154588', 'center', 700, FONT_SANS);
    R(g, x + 27, y + 38, 2.5, 7, '#d8dde2'); // ручка
    // ёлочка-ароматизатор на ручке
    R(g, x + 28, y + 45, 0.5, 3, '#ddd'); g.fillStyle = '#2f9a4a'; g.beginPath(); g.moveTo(x + 28.2, y + 47); g.lineTo(x + 25.5, y + 53); g.lineTo(x + 31, y + 53); g.fill();
    // бумажка на скотче
    g.save(); g.translate(x + 1, y + 44); g.rotate(-0.12);
    R(g, 0, 0, 16, 9, '#fbe98f'); T(g, 'ЕВРОРЕМОНТ', 8, 3, 2.6, '#333', 'center', 700, FONT_SANS); T(g, 'В 2027', 8, 6.3, 2.8, '#b3261e', 'center', 700, FONT_SANS);
    R(g, 5, -1, 6, 2, 'rgba(230,230,220,0.8)'); g.restore();
    // основание
    R(g, x - 1, y + 61, w + 2, 2, '#1a3a70');
  }
