...
(() => {
  ...
  function drawWalls(g) {
    ...
    R(g, 604, 404, 36, 9, '#c9a640'); R(g, 605, 405, 34, 7, '#e8cf7a'); T(g, 'Ф.П.', 622, 408.8, 5.6, '#3b2a10');
  ...
  function drawCounter(g) {
    ...
    note(g, 136, y - 9, 14, 5, ['НА КОФЕ'], { noTape: true, size: 3.4 });
    // Чайник
  ...
  function buildProps() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      prop(WD.LEFT, y0 - 26, 40, 72, y0 + 42, g => {
        ...
        for (let k = 0; k < 3; k++) {
          ...
          R(g, x + 12, y0 + 6 + k * 13.3, 10, 2, '#c8ced0');
          note(g, x + 4, y0 + 3 + k * 13.3, 7, 4, [String(2014 + i * 3 + k)], { noTape: true, size: 3, bg: '#fff' });
        }
    ...
    prop(310, 420, 40, 36, 452, g => {
      ...
      T(g, '!', 341, 441, 7, '#222');
      note(g, 326, 424, 30, 7, ['МОКРЫЙ ПОЛ'], { noTape: true, size: 3.8, bg: '#f2c230' });
    }, 'bucket');
    ...
    prop(654, 438, 104, 58, 492, g => {
      ...
      T(g, 'Ф.П.', x + 46, y + 25.6, 4.6, '#3b2a10');
      papers(g, x + 26, y + 6, 3);
    }, 'boss_desk');
    ...
    for (const [ry, id] of [[364, 'rack_row1'], [476, 'rack_row2']]) {
      prop(824, ry - 30, 120, ry === 476 ? 80 : 60, ry + (ry === 476 ? 46 : 26), g => {
        for (let i = 0; i < 4; i++) {
          const x = 828 + i * 28;
          R(g, x, ry - 26, 26, 26, '#1c2427'); R(g, x, ry - 26, 26, 2, '#3a464a');
          R(g, x, ry, 26, ry === 476 ? 46 : 26, '#141b1e');
          for (let y = ry + 2; y < ry + (ry === 476 ? 44 : 24); y += 5) { R(g, x + 2, y, 22, 4, '#222c30'); R(g, x + 3, y + 1.5, 10, 1, '#3e4c52'); }
        }
        // кабели-спагетти
        g.strokeStyle = '#2a6ab0'; g.lineWidth = 0.8;
        for (let k = 0; k < 6; k++) { g.beginPath(); g.moveTo(830 + k * 18, ry - 24); g.quadraticCurveTo(840 + k * 16, ry - 34, 850 + k * 14, ry - 22); g.stroke(); }
        g.strokeStyle = '#e0a020';
        for (let k = 0; k < 3; k++) { g.beginPath(); g.moveTo(835 + k * 30, ry - 20); g.quadraticCurveTo(850 + k * 28, ry - 30, 870 + k * 24, ry - 18); g.stroke(); }
        if (ry === 364) note(g, 836, ry + 6, 44, 12, ['НЕ ВЫКЛЮЧАТЬ!!!', 'ПРОД'], { size: 4.6, fg: '#b3261e' });
      }, id);
    }
