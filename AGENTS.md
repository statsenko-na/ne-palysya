...
`Не палься` — локальная браузерная top-down pixel-art стелс-игра про Викентия (айтишник-рокер без очков) в отделе кредитных рисков алматинского БЦ «Угар». Начальник — Директор Начальникович (Д.Н., лысый, в очках, без усов). Игровой UI использует стилизованное название «Фирдом Банк» и эмблему без словесного логотипа.
...
- `assets/` — спрайты v2 (Викентий, Директор, коллеги), вид на Алматы, SVG-эмблема; `*-v1.png` — исходники для пайплайна `scripts/normalize_sprites.py`.
